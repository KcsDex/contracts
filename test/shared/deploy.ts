...
import KDEX from '../../build/KDEX.json'
...
export async function deployKDEX(
  wallet: Wallet,
  name = TOKEN_NAME,
  symbol = TOKEN_SYMBOL,
  totalCap = TOTAL_CAP,
  ...
  const token = await deployContract(wallet, KDEX, [name, symbol, totalCap, manualMintLimit, lockFromBlock, lockToBlock])
  return token
}
