...
import { deployMasterBreeder, deployKDEX, TOKEN_NAME, TOKEN_SYMBOL, TOTAL_CAP, MANUAL_MINT_LIMIT } from '../shared/deploy'
...
describe('MasterBreeder::Rewards', () => {
  ...
  beforeEach(async () => {
    govToken = await deployKDEX(alice, TOKEN_NAME, TOKEN_SYMBOL, TOTAL_CAP, MANUAL_MINT_LIMIT, LOCK_FROM_BLOCK, LOCK_TO_BLOCK)
  })
  ...
  context("Entering & withdrawing from pools + claiming rewards", function () {
    ...
    it("should pay out VIPER rewards after farming has started", async function () {
      ...
      await expect(breeder.connect(bob).claimReward(0))
        .to.emit(breeder, 'SendKDEXReward') // emit SendKDEXReward(msg.sender, _pid, pending, lockAmount);
