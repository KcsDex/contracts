...
// MasterBreeder is the master breeder of whatever creature the KDEX represents.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once KDEX is sufficiently
...
contract MasterBreeder is Ownable, Authorizable, ReentrancyGuard {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of KDEXs
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint; // How many allocation points assigned to this pool. KDEXs to distribute per block.
        uint256 lastRewardBlock; // Last block number that KDEXs distribution occurs.
        uint256 accGovTokenPerShare; // Accumulated KDEXs per share, times 1e12. See below.
    }

    // The Governance token
    KDEX public govToken;
    ...
    // KDEXs created per block.
    uint256 public REWARD_PER_BLOCK;
    // Bonus muliplier for early KDEX makers.
    ...
    // The block number when KDEX mining starts.
    ...
    // Update the given pool's KDEX allocation point. Can only be called by the owner.
    function set(
        uint256 _pid,
        uint256 _allocPoint,
        bool _withUpdate
        ...
        )
    {
        ...
        uint256 KDEXCanMint = govToken.cap().sub(govToken.totalSupply());

        if (KDEXCanMint < amount) {
            forDev = 0;
            forFarmer = KDEXCanMint;
            forLP = 0;
            forCom = 0;
            forFounders = 0;
        } else {
    ...
    // View function to see pending KDEXs on frontend.
    ...
    function _harvest(uint256 _pid) internal {
        ...
        if (user.amount > 0) {
            ...
            if (pending > 0) {
                ...
                emit SendKDEXReward(msg.sender, _pid, pending, lockAmount);
            }
    ...
    // Deposit LP tokens to MasterBreeder for KDEX allocation.
    function deposit(
        uint256 _pid,
        uint256 _amount,
        address _ref
